...
The variable Aircraft was split into Aircraft Make and Aircraft Model using the Separate (REF??) function. The consistency of the values stored in these two new variables was improved using three Mutate (REF??) operations with if_else fucntions (REF??). String Replace and Remove (REF??) allowed for the values in the new variables to be improved by removing non-alphanumeric characters and giving a single string value without additional characters of sub-model descriptors such as "-200LR", "-300W". 

Filtering was applied to the Aircraft Make variable as the questions posed relate to Boeing and Airbus. 
